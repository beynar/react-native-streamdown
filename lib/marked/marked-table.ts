...
interface BaseCell {
  ...
  text: string;
  // The raw markdown source of the cell
  raw: string;
...
export const splitCells = (
  ...
  return processSpans(cells, count, prevRow || [], maxColspan, tableRow);
};

// Extract raw cell content from the original row for a specific cell index
function extractRawCellContent(
  originalRow: string,
  cellIndex: number,
  totalCells: number
): string {
  ...
  // Split the original row by pipes to get raw cell content
  const rawCells = splitRow(originalRow);

  // Remove first/last cell if it's empty (from leading/trailing pipes)
  if (rawCells.length > 0 && !rawCells[0]) rawCells.shift();
  if (rawCells.length > 0 && !rawCells[rawCells.length - 1]) rawCells.pop();

  // Return the raw content for the specific cell index
  return cellIndex < rawCells.length ? rawCells[cellIndex] : "";
}
...
const processSpans = (
  ...
  const colspanCells = new Map<
    string,
    { original: WorkingCell; newCell: WorkingCell }
  >();
  ...
  for (i = 0; i < cells.length; i++) {
    ...
    // Extract raw content for this cell from the original row
    const rawCellContent = extractRawCellContent(
      originalRow,
      cellIndex,
      cells.length
    );
  ...
  for (i = 0; i < processedCells.length; i++) {
    ...
    const isRowspanIndicator =
      cellText.slice(-1) === "^" && !cellText.match(/\^[^^\n\r]+\^$/); // Not a superscript pattern ^text^
      ...
      for (j = 0; j < prevRow.length; j++) {
        ...
        if (
          ...
          if (cell.colspan > 1 && prevCell.colspan > 1) {
            ...
            if (
              cell.colspan === prevCell.colspan &&
              cell.position === prevCell.position
            ) {
              ...
              if (
                ...
                cell.rowSpanTarget.text =
                  targetText + (targetText ? " " : "") + textToAppend;
              }
...
function processRows(
  ...
  if (processedHeaderRows.length > 0) {
    const theadRows: THeadRow[] = processedHeaderRows.map((row, index) => ({
      type: "tr",
      raw: headerRows[index] || "",
      tokens: row.map((cell) => {
        // Use the cell's position to get the correct alignment
        const cellAlignment =
          cell.position !== undefined ? alignment[cell.position] : null;
  ...
  if (bodyRows.length > 0) {
    ...
    for (let i = 0; i < bodyRows.length; i++) {
      const prevRow =
        i > 0
          ? processedBodyRows[i - 1]
          : processedHeaderRows[processedHeaderRows.length - 1];
    ...
    if (tbodyRows.length > 0) {
      const tbodyStartIndex = 0;
      const tbodyEndIndex = bodyRows.length - (tfootRows.length > 0 ? 1 : 0);
      const tbodyRowTokens: TRow[] = tbodyRows.map((row, index) => ({
        type: "tr",
        raw: bodyRows[tbodyStartIndex + index] || "",
        tokens: row.map((cell) => {
          // Use the cell's position to get the correct alignment
          const cellAlignment =
            cell.position !== undefined ? alignment[cell.position] : null;
      ...
      tokens.push({
        type: "tbody",
        raw: bodyRows.slice(tbodyStartIndex, tbodyEndIndex).join("\n"),
    ...
    if (tfootRows.length > 0) {
      const tfootRowTokens: TRow[] = tfootRows.map((row, index) => {
        const originalRowIndex = bodyRows.length - tfootRows.length + index;
        return {
          type: "tr",
          raw: bodyRows[originalRowIndex] || "",
          tokens: row.map((cell) => {
            // Use the cell's position to get the correct alignment
            const cellAlignment =
              cell.position !== undefined ? alignment[cell.position] : null;
      ...
      tokens.push({
        type: "tfoot",
        raw: bodyRows.slice(bodyRows.length - tfootRows.length).join("\n"),
        ...
        tokenizer(this: TokenizerThis, src: string): any {
          ...
          if (hasHeaderAlignment) {
            ...
            if (headerEndIndex === -1) {
              ...
              bodyRows =
                headerEndIndex + 1 < allRows.length
                  ? allRows.slice(headerEndIndex + 1)
                  : [];
            ...
            bodyRows = allRows;
            const firstRowCells = bodyRows[0]
              .split("|")
          ...
          if (detectFooter && hasHeaderAlignment && bodyRows.length > 0) {
            ...
            for (let i = bodyRows.length - 1; i >= 0; i--) {
              ...
              if (/^ *\| *:?-+:? *(\| *:?-+:? *)*\| *$/.test(row)) {
                ...
                processedBodyRows = bodyRows
                  .slice(0, i)
                  .concat(bodyRows.slice(i + 1));
                break;
              }
            }
          }
